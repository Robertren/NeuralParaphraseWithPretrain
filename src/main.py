...
def model_inference(model_list, sentence_data_index_a, sentence_data_index_b, char_embedding):
    # unzip the models
    ...
    sentence_data_a = get_sentence_embedding(char_embedding, index=sentence_data_index_a)
    sentence_data_b = get_sentence_embedding(char_embedding, index=sentence_data_index_b)
...
def test_model(loader, model_list, k, cuda, char_embedding):
    ...
    for iter, (sentence_data_index_a, sentence_data_index_b, label) in enumerate(loader):
        ...
        outputs = model_inference(model_list, sentence_data_index_a, sentence_data_index_b, char_embedding)
        predicted = (outputs.data > 0.5).long().view(-1)
        total += label_batch.size(0)
        correct += (predicted == label_batch.data).sum()
        # print(torch.topk(outputs.data, k, dim=0))
        # predicted = torch.topk(outputs.data, k, dim=0)[0]
